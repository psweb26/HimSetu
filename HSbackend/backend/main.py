...
from security import create_access_token, hash_password, require_admin_user, verify_password
import models
from models import (
    Asset,
    Incident,
...
def staff_and_executive_login(
    ...
    token = create_access_token(officer)
...
def current_staff_profile(current_officer: Officer = Depends(require_admin_user)) -> Dict[str, Any]:
    source_role = enum_value(current_officer.role)
    return {
...
def community_discovery(db: Session = Depends(get_db)) -> List[GrievanceResponse]:
    ...
    tickets = [
        ticket
        for ticket in db.query(Grievance).order_by(Grievance.created_at.desc()).all()
        if not ticket.incident or not any(
            transition.actor_type == "Admin"
            and transition.reason
            and "moderation: hidden" in transition.reason.lower()
...
def public_locations(db: Session = Depends(get_db)) -> List[Dict[str, Any]]:
    records = db.query(District).order_by(District.name.asc()).all()
    return [
        {
            "id": district.id,
            "name": district.name,
            "hierarchy": HIMACHAL_ADMIN_HIERARCHY.get(district.name, {}),
        }
...
def executive_alerts(
    db: Session = Depends(get_db),
    _admin: Officer = Depends(require_admin_user),
) -> Dict[str, Any]:
    promote_high_upvote_tickets(db)
    metric_row = db.query(
...
# The admin portal is another client of this FastAPI application. These routers
# deliberately share the public models, database session, uploads, and domain helpers.
from routers.admin.community import router as admin_community_router
from routers.admin.complaints import router as admin_complaints_router
from routers.admin.dashboard import router as admin_dashboard_router
from routers.admin.heritage import router as admin_heritage_router
from routers.admin.locations import router as admin_locations_router
from routers.admin.media import router as admin_media_router
from routers.admin.operations import router as admin_operations_router
...
for _admin_route_group in (
    admin_dashboard_router,
    admin_complaints_router,
    admin_community_router,
    admin_users_router,
    admin_operations_router,
    admin_heritage_router,
    admin_locations_router,
    admin_media_router,
    admin_settings_router,
):
    app.include_router(_admin_route_group, prefix="/api/admin")
